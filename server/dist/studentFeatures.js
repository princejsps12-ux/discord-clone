...
exports.registerStudentFeatures = registerStudentFeatures;
exports.registerStudySocketHandlers = registerStudySocketHandlers;
const aiService_1 = require("./aiService");
const streak_1 = require("./streak");
function registerStudentFeatures(app, prisma, io, deps) {
    ...
    app.post("/api/ai/ask", auth, async (req, res) => {
        ...
        if (!channelId || !question?.trim())
            return res.status(400).json({ error: "channelId and question required" });
        ...
        const answer = await (0, aiService_1.answerStudentQuestion)(question.trim(), context);
    ...
    app.patch("/api/messages/:messageId/meta", auth, async (req, res) => {
    ...
    app.post("/api/messages/:messageId/bookmark", auth, async (req, res) => {
        const existing = await prisma.message.findUnique({ where: { id: param(req, "messageId") } });
        if (!existing)
            return res.status(404).json({ error: "Not found" });
        const member = await prisma.member.findUnique({
    ...
    app.delete("/api/messages/:messageId/bookmark", auth, async (req, res) => {
        await prisma.bookmark.deleteMany({ where: { userId: req.userId, messageId: param(req, "messageId") } });
        res.json({ ok: true });
    });
    app.get("/api/servers/:serverId/notes", auth, async (req, res) => {
    ...
    app.post("/api/servers/:serverId/notes", auth, async (req, res) => {
        const ok = await prisma.member.findUnique({
            where: { userId_serverId: { userId: req.userId, serverId: param(req, "serverId") } },
        });
        if (!ok)
            return res.status(403).json({ error: "Forbidden" });
        const { title, fileUrl, mimeType, channelId } = req.body;
        if (!title || !fileUrl)
            return res.status(400).json({ error: "title and fileUrl required" });
        if (channelId) {
    ...
    app.post("/api/channels/:channelId/study-sessions", auth, async (req, res) => {
        const { title, plannedMinutes } = req.body;
        const channel = await prisma.channel.findUnique({ where: { id: param(req, "channelId") } });
        if (!channel)
            return res.status(404).json({ error: "Not found" });
        const member = await prisma.member.findUnique({
        ...
        (0, streak_1.bumpUserStreak)(prisma, req.userId).catch(() => undefined);
        res.status(201).json(session);
    });
    app.post("/api/study-sessions/:sessionId/join", auth, async (req, res) => {
    ...
    app.patch("/api/study-sessions/:sessionId/end", auth, async (req, res) => {
        const session = await prisma.studySession.findUnique({ where: { id: param(req, "sessionId") } });
        if (!session)
            return res.status(404).json({ error: "Not found" });
        const member = await prisma.member.findUnique({
        ...
        for (const p of participants) {
            await (0, streak_1.addStudyMinutes)(prisma, p.member.userId, minutes / Math.max(1, participants.length));
        }
        io.to(session.channelId).emit("study:session", { action: "end", sessionId: session.id });
        res.json({ ok: true, minutes: Math.round(minutes) });
    });
    app.get("/api/channels/:channelId/study-sessions/active", auth, async (req, res) => {
        const channel = await prisma.channel.findUnique({ where: { id: param(req, "channelId") } });
        if (!channel)
            return res.status(404).json({ error: "Not found" });
        const member = await prisma.member.findUnique({
    ...
    app.get("/api/streak/me", auth, async (req, res) => {
    ...
    app.get("/api/servers/:serverId/leaderboard", auth, async (req, res) => {
        const ok = await prisma.member.findUnique({
            where: { userId_serverId: { userId: req.userId, serverId: param(req, "serverId") } },
        });
        if (!ok)
    ...
    app.get("/api/servers/:serverId/analytics", auth, async (req, res) => {
        const ok = await prisma.member.findUnique({
            where: { userId_serverId: { userId: req.userId, serverId: param(req, "serverId") } },
        });
        if (!ok)
...
function registerStudySocketHandlers(socket, io, userId) {
    socket.on("study:join", (p) => {
        const payload = p;
        if (!payload?.sessionId || !payload?.channelId)
            return;
    ...
    socket.on("study:leave", (p) => {
        const payload = p;
        if (!payload?.sessionId)
            return;
        socket.leave(`study:${payload.sessionId}`);
        if (payload.channelId)
            io.to(payload.channelId).emit("study:presence", { sessionId: payload.sessionId, userId, action: "leave" });
    });
}
