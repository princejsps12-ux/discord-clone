 ...
 */
export async function answerStudentQuestion(question: string, context?: string): Promise<string> {
      ...
      const res = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            ...(context ? [{ role: "user", content: `Channel context: ${context.slice(0, 500)}` } as const] : []),
...
export async function sahayakSummarizeChat(transcript: string): Promise<string> {
  ...
  if (key) {
    const text = await openaiChat(
      `${SAHAYAK_SYSTEM}\nYour task now: summarize the conversation clearly in Hinglish.`,
      userBlock,
    );
    if (text) return text;
  }
...
export async function sahayakAnswerFromHistory(transcript: string, question: string): Promise<string> {
  const clipped = clipTranscript(transcript);
  const userBlock = `Chat history (oldest to newest):\n${clipped}\n\n---\nUser question:\n${question.slice(0, 8000)}`;
  const key = process.env.OPENAI_API_KEY;
  if (key) {
    const text = await openaiChat(
      `${SAHAYAK_SYSTEM}\nUse the chat history when relevant. If the answer is not in history, still help with general knowledge appropriate for students.`,
      userBlock,
    );
    if (text) return text;
  }
